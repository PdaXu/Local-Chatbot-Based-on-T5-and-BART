...
class ChatApp(QWidget):
    ...
    def show_about(self):
        ...
        browser.append("\n### T5模型介绍")
        browser.append("T5（Text-to-Text Transfer Transformer）是Google提出的一种基于Transformer架构的预训练语言模型。它将所有自然语言处理任务都统一为文本到文本的形式，例如翻译、问答、摘要等。T5模型通过在大规模无监督数据上进行预训练，学习到了通用的语言表示，然后在特定任务上进行微调，从而在多个自然语言处理任务中取得了很好的效果。")

        browser.append("\n### BART模型介绍")
        browser.append("BART（Bidirectional and Auto-Regressive Transformer）是Facebook AI Research提出的一种序列到序列的预训练模型。它结合了双向编码器（如BERT）和自回归解码器（如GPT）的优点。BART通过在大规模无监督数据上进行预训练，学习到了文本的双向上下文信息和生成能力。在微调阶段，BART可以用于各种自然语言生成和理解任务，如文本摘要、机器翻译、问答系统等。")
